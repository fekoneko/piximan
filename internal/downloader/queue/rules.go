...
func (r *Rules) MatchWork(w *work.Work, partial bool) (matches bool, warnings []error) {
	matches = matchManyToOne(r.Ids, w.Id, "ids", "id", &warnings) &&
		!matchManyToOne(r.NotIds, w.Id, "not_ids", "id", &warnings) &&
		matchSubstrings(r.TitleContains, w.Title, "title_contains", "title", &warnings) &&
		!matchSubstrings(r.TitleNotContains, w.Title, "title_not_contains", "title", &warnings) &&
		...
		!matchSubstrings(r.DescriptionNotContains, w.Description, "description_not_contains", "description", &warnings) &&
		...
		!matchManyToOne(r.NotUserIds, w.UserId, "not_user_ids", "user id", &warnings) &&
		matchManyToOne(r.UserNames, w.UserName, "user_names", "user name", &warnings) &&
		!matchManyToOne(r.NotUserNames, w.UserName, "not_user_names", "user name", &warnings) &&
		...
		!matchManyToOne(r.NotSeriesIds, w.SeriesId, "not_series_ids", "series id", &warnings) &&
		matchSubstrings(r.SeriesTitleContains, w.SeriesTitle, "series_title_contains", "series title", &warnings) &&
		!matchSubstrings(r.SeriesTitleNotContains, w.SeriesTitle, "series_title_not_contains", "series title", &warnings) &&
		...
		!matchManyToMany(r.NotTags, w.Tags, "not_tags", "tags", &warnings)
...
func matchDefined[T comparable](
	r *bool, f *T, rule string, field string, warnings *[]error, warnUndefined bool,
) bool {
	if r == nil {
		if f == nil && warnUndefined {
			*warnings = append(*warnings, fmt.Errorf("ignoring rule '%v': %v is unknown", rule, field))
		}
		return true
	}
	return (*r && f != nil) || (!*r && f == nil)
}
