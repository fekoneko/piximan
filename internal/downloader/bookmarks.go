...
func (d *Downloader) ScheduleMyBookmarks(
	kind queue.ItemKind, tag *string, from *uint64, to *uint64, olderTime *time.Time, newerTime *time.Time,
	...
	d.crawlQueue = append(d.crawlQueue, func() error {
		...
		d.ScheduleBookmarks(
			userId, kind, tag, from, to, olderTime, newerTime,
...
func (d *Downloader) ScheduleBookmarks(
	userId uint64, kind queue.ItemKind, tag *string, from *uint64, to *uint64, olderTime *time.Time,
	newerTime *time.Time, private bool, size image.Size, onlyMeta bool, lowMeta bool, paths []string,
) {
	if olderTime != nil && newerTime != nil && olderTime.After(*newerTime) {
		err := fmt.Errorf("newer time boundary (%v) is after older time boundary (%v)", newerTime, olderTime)
	...
	d.crawlQueue = append(d.crawlQueue, func() error {
		...
		if olderTime == nil && newerTime == nil {
			d.processBookmarksParallel(
				userId, kind, tag, fromOffset, toOffset, olderTime,
				newerTime, private, size, onlyMeta, lowMeta, paths,
			)
		} else if olderTime == nil && newerTime != nil {
			if !hasNewerTime {
...
func (d *Downloader) fetchBookmarksAndScheduleWorks(
	userId uint64, kind queue.ItemKind, tag *string, offset uint64, limit uint64, olderTime *time.Time,
	newerTime *time.Time, private bool, size image.Size, onlyMeta bool, lowMeta bool, paths []string,
	...
	for _, result := range results {
		if result.BookmarkedTime == nil && olderTime != nil && newerTime != nil {
			...
			logext.Error("%v %v: %v", bookmarksLogMessage("failed to schedule", userId, tag, &offset), kind, err)
		} else if olderTime != nil && result.BookmarkedTime.Before(*olderTime) {
			hasOlderTime = true
		} else if newerTime != nil && result.BookmarkedTime.After(*newerTime) {
			hasNewerTime = true
		} else {
			d.ScheduleWithKnown(
