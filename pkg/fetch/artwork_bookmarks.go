...
func ArtworkBookmarksAuthorized(
	...
	url := fmt.Sprintf("https://www.pixiv.net/ajax/user/%v/illusts/bookmarks?tag=%v&offset=%v&limit=%v&rest=show", userId, tag, offset, limit)
	body, err := Do(client, url, nil)
	if err != nil {
		return nil, err
	}

	var unmarshalled dto.Response[dto.BookmarkArtworksBody]
